...
//
//////////////////////////////////////////////////////////////////////////////////
// Description: Testbench for the Predictor FSM of the spec_prefill_unit module
...
module spec_prefill_unit_predictor_tb;
    ...
    localparam int unsigned NUM_ACCESS = HDC_DIM / NUM_INPUT; // 64
    ...
    task automatic wait_for_predictor_status(input logic [1:0] expected_status, input int max_cycles);
        ...
        while (count < max_cycles) begin
            ...
            if (status_data[3:2] == expected_status) begin
                return;
            end
            @(posedge clk_i);
            count++;
        end
        $error("Timeout waiting for predictor status 0x%02h", expected_status);
    endtask
    ...
    // Test: Predictor POWER_POWER_TEST mode (only asserts power_test, does not actually start predictor_en)
    task automatic test_predictor_power_test();
        $display("\n=== Test 0: Predictor POWER_POWER_TEST Mode ===");
        test_count++;

        // Configure CSR: cmd=CMD_IDLE, predictor_power_mode=POWER_POWER_TEST
        // cmd=00, encoder_power=00, predictor_power=10->[5:4]=2 => 0x20
        write_data = 64'h00000020;
        axi_write(CSR_BASE_ADDRESS, write_data);
        $display("[%0t] Set predictor POWER_POWER_TEST mode, CSR=0x%016h", $time, write_data);
    ...
    task automatic test_predictor_basic();
        ...
        $display("[%0t] Writing Weight SRAM data for agent_id=0 (sram_addr, part2|part1|part0)...", $time);
        ...
        // cmd=CMD_PREDICTOR(10), predictor_power_mode=POWER_WORK(01), other power_mode=IDLE, agent_id=0
        // Bit layout: cmd[1:0], encoder_power[3:2], predictor_power[5:4], learner_power[7:6], agent_id[9:8]
        // Therefore: cmd=2, predictor_power=1<<4=16, total = 2 + 16 = 0x12
        write_data = 64'h00000012;
        axi_write(CSR_BASE_ADDRESS, write_data);
    ...
    task automatic wait_for_learner_status(input logic [1:0] expected_status, input int max_cycles);
        ...
        $error("Timeout waiting for learner status 0x%02h", expected_status);
    endtask
    ...
    task automatic test_predictor_multiple_agent();
        ...
        // Extract token IDs and compute the expected encoder output
        for (int j = 0; j < TOKEN_WINDOW; j++) begin
            token_ids[j] = write_data[TOKEN_ID_WIDTH*j +: TOKEN_ID_WIDTH];
        end
        encoder_vector = compute_encoder_output(token_ids);
        ...
        for (agent = 0; agent < NUM_AGENT; agent++) begin
            ...
            // cmd=CMD_PREDICTOR(10), predictor_power_mode=POWER_WORK(01), other power_mode=IDLE, agent_id=agent
            // cmd=2, predictor_power=1<<4=16, agent_id=(agent<<8)
            write_data = 64'h00000012 | (agent << 8);
            axi_write(CSR_BASE_ADDRESS, write_data);
            // After predictor_en is asserted, the predictor FSM starts feeding the first set of data to the core only in the next cycle
            repeat (2) @(posedge clk_i);
    ...
    initial begin
        ...
        $display("========================================");
        $display("Predictor Testbench Started");
        $display("========================================");
        
        // Run tests

        test_predictor_multiple_agent();
        #4;
        // test_predictor_power_test();
        // #4;        
        // Test summary
        $display("\n========================================");
