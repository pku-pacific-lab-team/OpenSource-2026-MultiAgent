...
// Date:   06.06.2019
//
// Modified by the PKU_2602 project team (School of Integrated Circuits, Peking University), last changed 2025-12-29:
// adapted for the PKU_2602 SoC integration (61 differing lines against the upstream version; see the file inventory in THIRD_PARTY.md at the repository root).
// Upstream: axi_llc (https://github.com/pulp-platform/axi_llc), src/hit_miss_detect/axi_llc_tag_store.sv at tag v0.2.2
...
module axi_llc_tag_store #(
  ...
  for (genvar i = 0; unsigned'(i) < Cfg.SetAssociativity; i++) begin : gen_tag_macros
    ...
    tag_data_t ram_expected; // expected comparison result of tags

    ////////////////////////////////////////////////////////////////////////////////
    // Modified by: Mingxuan Li <siris-lmx@stu.pku.edu.cn> [Peking University]
    // Use the bit-maskable SRAM wrapper of the die (tech_specific/tc_sram.v) instead of the generic tc_sram
    logic [64-1:0] wdata_aligned;
    logic [64-1:0] rdata_aligned;
    assign wdata_aligned = {'0, ram_wdata};
    assign ram_rdata     = rdata_aligned[TagDataLen-1:0];

    rf_be_256x64 i_tag_store (
      .clk_i          ( clk_i         ),
      .chip_enable_i  ( ram_req[i]    ),
      .write_enable_i ( ram_we[i]     ),
      .addr_i         ( ram_index     ),
      .write_data_i   ( wdata_aligned ),
      .bit_enable_i   ( '1            ),
      .read_data_o    ( rdata_aligned )
    );
    ...
    // comparator (XNOR)
    assign ram_expected = tag_data_t'{
          val: bist_pattern.val,
          dit: bist_pattern.dit,
          tag: (req_q.mode == axi_llc_pkg::Bist) ? bist_pattern.tag : req_q.tag
        };
    assign ram_compared = ram_expected ~^ram_rdata;
