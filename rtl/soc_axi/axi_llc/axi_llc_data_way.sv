...
// adapted for the PKU_2602 SoC integration (53 differing lines against the upstream version; see the file inventory in THIRD_PARTY.md at the repository root).
// Upstream: axi_llc (https://github.com/pulp-platform/axi_llc), src/axi_llc_data_way.sv at tag v0.2.2
...
module axi_llc_data_way #(
  ...
  logic [Cfg.BlockSize-1:0] bit_enable;
  for (genvar i = 0; i < Cfg.BlockSize/8; i++) begin
    assign bit_enable[i*8 +: 8] = {8{inp_i.strb[i]}};
  end

  sram_be_1024x64 i_data_sram (
    .clk_i          ( clk_i      ),
    .chip_enable_i  ( ram_req    ),
    .write_enable_i ( inp_i.we   ),
    .addr_i         ( addr       ),
    .write_data_i   ( inp_i.data ),
    .bit_enable_i   ( bit_enable ),
    .read_data_o    ( out_o.data )
  );
