...
module axi_to_reg_v2 #(
  ...
  // Register endpoints never report exclusive success (mem_exokay_i is tied low).
  // An all-zero write strobe can yield EXOKAY in the generic bank reduction;
  // normalize that success code here while preserving IDs, errors and handshakes.
  axi_rsp_t mem_axi_rsp;
  always_comb begin
    axi_rsp_o = mem_axi_rsp;
    if (mem_axi_rsp.b.resp == axi_pkg::RESP_EXOKAY)
      axi_rsp_o.b.resp = axi_pkg::RESP_OKAY;
  end
