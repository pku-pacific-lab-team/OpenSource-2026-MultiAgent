...
//
// Modified by the PKU_2602 project team (School of Integrated Circuits, Peking University), last changed 2026-01-09:
// adapted for the PKU_2602 SoC integration (28 differing lines against the upstream version; see the file inventory in THIRD_PARTY.md at the repository root).
// Upstream: axi_llc (https://github.com/pulp-platform/axi_llc), src/axi_llc_config.sv at tag v0.2.2
...
module axi_llc_config #(
  ...
  //add customized initial
  set_asso_t spm_initial;
  assign spm_initial = set_asso_t'({{(Cfg.SetAssociativity/2){1'b0}}, {(Cfg.SetAssociativity/2){1'b1}}});
    ...
    unique case (flush_state_q)
      ...
      FsmPreInit: begin
        ...
        if (bist_valid_i) begin
          flush_state_d = FsmIdle;
          // // after bist, ALL WAYS with no errors will be set to CACHE
          // conf_regs_o.cfg_spm     = bist_res_i;

          // Modification:after bist, ALL WAYS with no errors will be set to SPM
          conf_regs_o.cfg_spm     = spm_initial;
          spm_lock_d 		  = spm_initial;
          // No load specified for CfgSpm, as per default the reg is loaded anyway.
          conf_regs_o.flushed     = spm_initial;
          conf_regs_o.flushed_en  = 1'b1;
        end
      end
      default : /*do nothing*/;
