...
//
// Modified by the PKU_2602 project team (School of Integrated Circuits, Peking University), last changed 2026-01-03:
// adapted for the PKU_2602 SoC integration (16 differing lines against the upstream version; see the file inventory in THIRD_PARTY.md at the repository root).
// Upstream: FlooNoC (https://github.com/pulp-platform/FlooNoC), hw/floo_pkg.sv at tag v0.6.0
...
package floo_pkg;
  ...
  localparam route_cfg_t RouteDefaultCfg = '{
    ...
    XYAddrOffsetY: 0,
    IdAddrOffset: 0,
    // --- BEGIN VCS COMPATIBILITY FIX ---
    // The original default value of 0 for NumSamRules and NumRoutes causes
    // invalid array range errors like [-1:0] in some compilers (e.g., VCS)
    // when modules that use these parameters are instantiated without explicitly
    // overriding them.
    // Changing the default to 1 creates a safe, 1-bit dummy array in such cases,
    // which is harmless for routing algorithms that do not use these tables (like XY).
    NumSamRules: 1,
    NumRoutes: 1
    // --- END VCS COMPATIBILITY FIX ---
  };
